...
README SHALL описывать назначение сервиса (справочные курсы и пересчёт сумм), платформу Flutter Web, источник Frankfurter с загрузкой в браузере без сервера и сохранение упорядоченного списка таблицы в `localStorage` сайта. README SHALL пояснять постоянную базу EUR и два результата одной суммы: из EUR в валюту строки и из валюты строки в EUR. Старый выбор базы SHALL описываться как игнорируемый при сохранении списка; сумма и курсы между посещениями не сохраняются. Инструкция менять базу для обратной конвертации SHALL отсутствовать. README SHALL сохранять раздел об ограничениях данных Frankfurter.

#### Scenario: New developer reads the overview
- **WHEN** читатель открывает README впервые
- **THEN** он понимает назначение, платформу, источник, постоянную базу EUR, смысл двух результатов и сохранение списка в браузере

#### Scenario: Переход со старой версии
- **WHEN** читатель ранее сохранил другую базу и свой список валют
- **THEN** README объясняет, что список сохраняется, расчёты выполняются относительно EUR, обратный результат доступен сразу без переключения базы
